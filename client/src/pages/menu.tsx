...
export default function Menu() {
  ...
  // Smart image preloading - only preload current and adjacent categories
  React.useEffect(() => {
    const currentImage = getCategoryBackgroundImage(actualCategory);
    
    // Immediately preload current category image
    const img = new Image();
    img.src = currentImage;
    
    // Progressively preload other images after a delay
    const timeoutId = setTimeout(() => {
      Object.values(categoryBackgroundImages)
  ...
  return (
    <>
      ...
      <div 
        className="fixed inset-0 w-full h-full bg-gray-100 bg-cover bg-center bg-no-repeat transition-opacity duration-300"
        style={{
